...
static bool Symbols_visitVar(SymbolTable* st, AST* var) {
   assert(var->stringVal);
   const char* name = var->stringVal;
   Symbol* existing = SymbolTable_get(st, name);
   if (existing && existing->type == SYM_FUNCTION) {
      return fail("Name is a function", name, var);
   } else if (!existing) {
...
static bool Symbols_visitExp(SymbolTable* st, AST* exp) {
   switch (exp->type) {
      case AST_ADD: {
         bool ok = Symbols_visitExp(st, exp->firstChild);
         if (!ok) return false;
         return Symbols_visitExp(st, exp->firstChild->nextSibling);
      }
      case LITNUM:
         return true;
      case ID:
         return Symbols_visitVar(st, exp);
      default:
         return fail("internal compiler error!", "?!", exp);
   }
}
...
static bool Symbols_visitAssign(SymbolTable* st, AST* assign) {
   ...
   return Symbols_visitExp(st, assign->firstChild->nextSibling);
}
